...
def show_menu():
    ...
    try:
        ...
        print("\n  Invalid number, program being terminated...\n")

        sys.exit(0)

def show_books(_cursor):
    #  This is the inner join query 
    _cursor.execute('book_id, book_name, author, details')

    # Retrieving the results from the cursor object 
    books = _cursor.fetchall()

    print("\n  -- DISPLAYING BOOK LISTING --")
    
    # Here it is iterating over the player data set and displaying all the results 
    for book in books:
...
def show_books_to_add(_cursor, _user_id):
    """ Querying the database for a list of books not already in the users wishlist """
...
try:
    ...
    user_selection = show_menu() # Showing the main menu 

    # If/While statement while the user's selection is not 4
    while user_selection != 4:

        # If the user selects option 1, call the show_books method and then display the books
        if user_selection == 1:
            show_books(cursor)
        ...
        if user_selection == 3:
            ...
            while account_option != 3:
                ...
                if account_option == 2:
                    ...
                    # Gets the entered book_id 
                    book_id = int(input("\n        Enter the id of the book you want to add: "))
                    
                    # Adds the selected book the users wishlist
    ...
    if err.errno == errorcode.ER_ACCESS_DENIED_ERROR:
        ...
        print("  The specified database does not exist anywhere")
